...
func (a *App) initListener(_ context.Context) {
	lis, err := net.Listen("tcp", config.AppConfig().GRPC.Address()) //nolint:noctx // net.Listen не требует контекст, адрес из конфига
	if err != nil {
...
func (a *App) initGRPCServer(ctx context.Context) {
	...
	// Вызываем API-геттер ДО регистрации closer'а gRPC-сервера.
	// Это запускает цепочку ленивой инициализации: API → Service → Repository → PGPool.
	// PGPool регистрирует себя в closer первым, gRPC-сервер — вторым.
	// Closer работает по LIFO: сначала остановится gRPC, потом закроется пул БД.
	api := a.di.InventoryAPI(ctx)
